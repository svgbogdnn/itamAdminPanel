...
from app.utils.models import Course, User, Attended_course,Lesson
from datetime import date
from app.utils.models import db
student_courses_blueprint=Blueprint('student_courses',__name__)

@student_courses_blueprint.route('/student/<int:id>/courses')
def student_courses(id):
    student=User.query.get(id)
    att_c=Attended_course.query.filter_by(student_id=student.id).all()
    att_courses={}
    for i in range(len(att_c)):
        att_courses.update({f"att_course_{i}":{"id": str(att_c[i].course_id),
                                       "name":str(att_c[i].course.name),
                                       'count': str(len(Attended_course.query.filter_by(course_id=att_c[i].course_id).all())),
    ...
    for i in range(len(aff_c_id)):
        ...
        aff_courses.update({f"aff_course_{i}":{"id": str(course.id),
                                       ...
                                       'count': str(len(Attended_course.query.filter_by(course_id=course.id).all())),
...
@student_courses_blueprint.route('/student/<int:id>/courses/<int:course_id>',methods=['GET','POST'])
def student_all_inf_about_course(id,course_id):
    if request.method=='POST':
        new_course_entry=Attended_course(course_id=course_id,student_id=id)
        try:
