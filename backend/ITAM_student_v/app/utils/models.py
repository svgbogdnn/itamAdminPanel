...
db = SQLAlchemy() #инициализация объекта SQLAlchemy с использованием приложения Flask
...
class Attended_course (db.Model):
    __tablename__='attended_courses'
    id=db.Column(db.Integer, primary_key=True)
    student_id = db.Column( db.Integer, nullable =False)
    course_id = db.Column( db.Integer, db.ForeignKey('courses.id'))
    #по имени таблицы во множественном числе
...
class Course(db.Model):
    ...
    # родительский элемент один ко многим
    #дочерняя бд, в дочерней бд обращаемся через(объект дочерней).сourse к родительской
    attended_courses=db.relationship('Attended_course',backref='course')
    #новая колонка дочернего элемента
